...
def get_true_cost_function(env_id, env_configs={}, agent_id=0, c_id=None, games_by_aids={}):
...
def wall_in(unsafe_states, obs, acs, codes):
    if isinstance(obs, list):
        return (obs in unsafe_states[codes])
    elif isinstance(obs, np.ndarray):
        costs = []
        for i in range(len(obs)):
            tmp = obs[i].tolist()
            costs.append(float(tmp in unsafe_states[codes]))
