            ...
            for i in range(env.num_envs):
                ...
                for record_info_name in record_info_names:
                    if record_info_name == 'ego_velocity_x':
                        ...
                        record_infos[record_info_name][cid].append(np.mean(_info[i]['lanebase_relative_position'][0]))
                    else:
                        record_infos[record_info_name][cid].append(np.mean(_info[i][record_info_name]))
                if not is_constraint[i]:
                    if _info[i]['lag_cost']:
                        is_constraint[i] = True
                    else:
